        ...
        <PopoverTrigger asChild>
          <svg id="Layer_2" viewBox="-1 -1 163 541">
            ...
            <g>
              <AnimatePresence>
                {PATHS.map(
                  ({ info, path }: { info: PathInfo; path: string }) => (
                    // base case: path not selected or hovered
                    <path
                ...
                {hoveredPath && (
                  ...
                  <motion.path
                    ...
                    d={
                      PATHS.find(({ info }) => info.id === hoveredPath.id)?.path
                    }
                  />
                )}
                {selectedPath && (
                  ...
                  <motion.path
                    ...
                    d={
                      PATHS.find(({ info }) => info.id === selectedPath.id)
                        ?.path
                    }
                  />
                )}
...
const useSelectedPath = () => {
  const [selectedPath, setSelectedPath] = useState<PathInfo>();
  const [hoveredPath, setHoveredPath] = useState<PathInfo>();
  const [isHovered, setIsHovered] = useState(false);
  const hoverTimeout = useRef<NodeJS.Timeout>();
  const [popoverContent, setPopoverContent] = useState<PathContent>();
  ...
  const onMouseEnter = (path: PathInfo) => {
  ...
  useEffect(() => {
    if (hoveredPath) {
      hoverTimeout.current = setTimeout(() => {
        setIsHovered(true);
        setPopoverContent(hoveredPath.content); // Update content when hovered
      }, DEBOUNCE_HOVER);
    } else {
      if (isHovered) {
        hoverTimeout.current = setTimeout(() => {
